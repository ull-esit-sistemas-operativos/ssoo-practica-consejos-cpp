...
```{.cpp}
SafeFD open_file(const std::string& path, int flags, mode_t mode)
{
...
```{.cpp}
SafeFD testFd = open_file("test.txt", O_RDONLY, 0);
...
Para asegurar que nunca intentaremos copiarlo, se puede eliminar el constructor de copia y el operador de asignación de la clase `SafeFD` anterior:
...
2. Eliminar el operador de asignación. Ya no se podrá asignar un `SafeFD` a otro usando el operado `=`.
...
public:
    ...
    SafeFD(SafeFD&& other) noexcept  // <1>
        : fd_{other.fd_}
    {
        other.fd_ = -1;
    }

    SafeFD& operator=(SafeFD&& other) noexcept  // <2>
    {
    ...
    [[nodiscard]] bool is_valid() const noexcept    // <3>
    {
        return fd_ >= 0;
    }
...
2. Operador de asignación de movimiento.
3. Método para comprobar si el descriptor de archivo es válido (mayor que 0).
...
if (condition)
{
    testFd = open_file("test.txt", O_RDONLY, 0);    // <2>
}
...
```
1. Objeto `SafeFD` con descriptor de archivo inválido (-1).
...
Al hacerlo es importante usar `std::move()` para mover el objeto `SafeFD` del objeto `std::expected` a la variable `fd`, puesto que hemos prohibido las copias de `SafeFD`.
...
open_file_result result = open_file("test.txt", flags, mode);       // <2>
if (result)
{
...
3. Es importante usar `std::move()` para mover el descriptor de archivo de `result` a `fd`.
...
En caso contrario, puedes ignorar el resto de este apartado.
...
Las excepciones `std::system_error`[^header_system_error] guardar el código de error `errno` en el objeto y permiten obtener un mensaje de texto descriptivo del error con el método `what()`.
Este mensaje descriptivo es equivalente al que se obtiene con `std::strerror(errno)`.

[^header_system_error]: `std::system_error` se declara en `<system_error>`.

El tercer argumento del constructor de `std::system_error` es un mensaje de texto que precede al mensaje descriptivo del código de error.
En el ejemplo se construye con el nombre del fichero de código fuente --usando el valor de la macro `__FILE__`-- y el número de línea --usando el valor de `__LINE__`-- donde se ha producido el error.
Esto puede ser muy útil para localizar rápidamente los errores al depurar el programa.
...
La función `parse_args()` devuelve un `std::expected`[^header_optional] para poder devolver la estructura de datos `program_options` en caso de éxito o un _enum_ `parse_args_errors` con el código de error en caso de fallo.

[^header_optional]: `std::optional` está disponible desde gcc-12 (C++23) y se declara en `<expected>`.
